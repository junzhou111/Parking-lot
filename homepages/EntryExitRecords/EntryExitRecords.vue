<template>
	...
	<view class="EntryExitRecords">
		...
		<!-- 车辆详情 -->
		<ExitRecord :caralllist="caralllist" @infos="infos"></ExitRecord>
	</view>
</template>
		...
		methods: {
			...
			async getposition() {
				...
				for (var i = 0; i <= res.data.data.length - 1; i++) {
					this.positions.list.push(res.data.data[i])
					this.position.list.push(res.data.data[i])
				}
			},
			// 获取车场出入口位置信息
